...
public class PhaseCompElm extends ChipElm
{
	...
	public void stamp()
	{
		int vn = CircuitElm.cirSim.circuit.getNodeCount() + this.pins[2].voltSource;
		CircuitElm.cirSim.circuit.stampNonLinear(vn);
		CircuitElm.cirSim.circuit.stampNonLinear(0);
	...
	public void doStep() throws CircuitAnalysisException
	{
		...
		else
		{
			...
			int vn = CircuitElm.cirSim.circuit.getNodeCount() + this.pins[2].voltSource;
			CircuitElm.cirSim.circuit.stampMatrix(vn, vn, 1);
		}
